...
/**
 * Structural shape needed for the group filter. Generic so callers
 * (apps/web's `InsightData` from `@/lib/ai/subagents/insights`, the
 * package's own `InsightBase`, or any user-defined extension) can
 * pass their row type without the leaf package having to import the
 * AI subagent types.
 */
interface InsightWithGroups {
	groups?: string[] | null;
}
