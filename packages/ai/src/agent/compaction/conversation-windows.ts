...
function getCompactionLevel(usageRatio: number, hasOldHistory: boolean): CompactionLevel | null {
	// Old history alone is enough to start soft compaction so we can summarize
	// before the conversation actually hits the hard token ceiling.
...
function keepNewestWithinBudget(
	...
	// Always preserve the newest message in a bucket when possible.
	// Without this, a single long recent/warm message can be dropped entirely
	// just because it exceeds the bucket budget, which defeats the point of
	// prioritizing fresher context over old history.
...
export function prepareConversationWindows(
	...
	// Tokenize once up front so the selection logic can work purely off
	// bucket priority and remaining budget.
	...
	// Recent and warm windows are preserved first; older history only competes
	// for whatever budget remains after fresher context is secured.
	...
	// Anything excluded from the active window becomes a compaction candidate
	// rather than being silently dropped, so shared.ts can summarize it async.
