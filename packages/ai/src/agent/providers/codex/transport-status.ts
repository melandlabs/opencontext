/**
 * Codex transport-status controller.
 *
 * Keeps transport-recovery UI scoped to one agent turn. Repeated reconnect
 * events update one presentation; every terminal path clears it.
 */
...
export function createCodexTransportStatusController(
	...
	return {
		handle(message) {
			if (
				message.type === "retry" &&
				(message.retryKind === "reconnecting" || message.retryKind === "fallback")
			) {
				const hasAttempt = typeof message.attempt === "number" && typeof message.maxAttempts === "number";
				presenter.show({
