...
export class OpenCodeAgent extends BaseAgent {
	...
	async *run(prompt: string, options?: AgentOptions): AsyncGenerator<AgentMessage> {
		...
		try {
			...
			yield* this.runOpenCodePrompt(prompt, cwd, options, session.abortController.signal);
		} catch (error) {
	...
	async *plan(prompt: string, options?: PlanOptions): AsyncGenerator<AgentMessage> {
		...
		try {
			...
			// TODO: bridge OpenCode permission events once the CLI exposes a
			// stable approval protocol. Until then, planning must never opt
			// into --auto.
	...
	private async *runOpenCodePrompt(
		...
		const providerConfig = normalizeOpenCodeProviderConfig(this.config.providerConfig);
...
function formatOpenCodeExitError(closeEvent: Extract<OpenCodeCliEvent, { type: "close" }>) {
...
async function materializeOpenCodeImages(cwd: string, options?: AgentOptions): Promise<string[]> {
	const images = options?.images?.filter((image) => Boolean(image.data)) ?? [];
	if (images.length === 0) return [];

	const inputDir = join(cwd, ".opencontext-inputs");
	await mkdir(inputDir, { recursive: true });
	const files: string[] = [];
	for (const [index, image] of images.entries()) {
		if (!image.data) continue;
		const file = join(inputDir, `image-${index + 1}.${imageExtension(image.mimeType)}`);
...
export const opencodeAgentPlugin: AgentPlugin = defineAgentPlugin({
	metadata: OPENCODE_METADATA,
	factory: (config: AgentConfig) => createOpenCodeAgent(config),
});
