...
/**
 * Vitest config that picks up only `*.integration.test.ts` files. These
 * tests talk to a real LLM (via `OPENCONTEXT_LLM_*` env vars) and are
 * skipped automatically when the credentials aren't present, so CI
 * without secrets won't fail.
 *
 * Run with:
