import type { InsertRssItem, RssSubscription } from "./normalize";
...
export function extractRssTags(
	...
	for (const cat of keywordTags.categories) categories.add(cat);

	// Categories from metadata extraction
	for (const cat of metadataTags.categories) categories.add(cat);

	// Default categories from subscription config
	if (tagConfig?.defaultCategories) {
		for (const cat of tagConfig.defaultCategories) categories.add(cat);
	}

	// sourceType default categories (if no category yet)
	if (categories.size === 0) {
		for (const cat of sourceTypeRules.defaultCategories) categories.add(cat);
	}
