...
const registeredCommands: Array<{ name: string; handler: (inv: unknown) => Promise<{ kind: "success" | "error"; text: string }> }> = [];
...
const ctx = {
	...
	get(name: string): unknown {
		...
		if (name === "commands") {
			return {
				register(definition: { name: string; handler: typeof registeredCommands[number]["handler"] }): () => void {
...
try {
	...
	process.stdout.write(`✔ apply(ctx, config) — registered ${registeredTools.length} tools, ${preStepListeners.length} pre-step listeners, ${registeredCommands.length} command, ${registeredSkills.length} skill\n`);
	...
	if (!registeredSkills[0]?.body.includes("Untrusted historical evidence") && !registeredSkills[0]?.body.includes("untrusted historical evidence")) {
		throw new Error("skill body missing trust-model framing");
	}
	process.stdout.write(`✔ skill '${registeredSkills[0].name}' registered (body ${registeredSkills[0].body.length} bytes)\n`);
	...
	if (doctorBody.probe?.ok !== true) throw new Error(`/oc doctor: probe not ok: ${JSON.stringify(doctorBody.probe)}`);
	process.stdout.write(`✔ /oc doctor — mode=lib, scope=smoke:scope, probe ok=true, recentCount=${doctorBody.recentCount}\n`);
	...
	const rem = (await remember.execute({ content: "The API key is sk-abcdefghijklmnopqrstuv and should never be stored" }, {})) as { ok: boolean; error?: { code: string } };
	if (rem.ok) throw new Error("oc_remember should reject secret-like content");
	if (rem.error?.code !== "secret_rejected") throw new Error(`oc_remember: expected secret_rejected, got ${rem.error?.code}`);

	const rem2 = (await remember.execute({ content: "remember the project name is dsh-opencontext" }, {})) as { ok: boolean; value?: { ids: string[] } };
	...
	const getResult = (await get.execute({ ids: [firstId] }, {})) as { ok: boolean; value?: { items: Array<{ id: string; content: string }> } };
	...
	const searchResult = (await search.execute({ query: "dsh-opencontext" }, {})) as { ok: boolean; value?: { hits: unknown[] } };
	if (!searchResult.ok) throw new Error(`oc_search: ${JSON.stringify(searchResult)}`);
	process.stdout.write(`✔ oc_search returned ${searchResult.value?.hits.length ?? 0} hit(s) for "dsh-opencontext"\n`);
	...
	const prep = (await prepare.execute({ query: "dsh-opencontext project" }, {})) as { ok: boolean; value?: { contextBlock: string; hits: number } };
	...
	process.stdout.write(`✔ oc_prepare_context produced a ${prep.value?.contextBlock.length}-byte evidence block (${prep.value?.hits} hits)\n`);
	...
	const cap = (await capture.execute({ content: "a captured snippet", sourceType: "smoke" }, {})) as { ok: boolean; value?: { id: string } };
	...
	if (preStepListeners.length !== 2) throw new Error(`expected 2 pre-step listeners, got ${preStepListeners.length}`);
	const userPayload = {
		messages: [
	...
	let nextDecision = { kind: "enter", messages: [{ role: "user", content: [{ type: "text", text: "what was the dsh-opencontext project name?" }] }] };
	const downstreamNext = async () => nextDecision;
	const recallResult = (await preStepListeners[0]!(userPayload, downstreamNext)) as { kind: string; messages: Array<{ meta?: { kind?: string } }> };
	...
	const rev = (await revise.execute({ id: firstId, content: "the project is dsh-opencontext, an Apache-2.0 DSH plugin", reason: "more accurate" }, {})) as { ok: boolean; value?: { deprecatedId: string; newId: string } };
	...
	const ret = (await retire.execute({ id: rev.value!.newId, reason: "smoke test cleanup" }, {})) as { ok: boolean; value?: { ok: boolean } };
	...
	process.stdout.write(`✔ /oc doctor after retire — recentCount=${doctor2Body.recentCount} (deprecated entries hidden by default)\n`);
