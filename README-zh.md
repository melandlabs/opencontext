...
- **Agent / LLM 平台团队** —— 在一个运行时后面把记忆、检索与集成接进产品
- **产品工程师** —— 给现有应用补一个 agentic 界面(聊天、搜索、自动化), 而不用从零重建存储与连接器
- **内部工具团队** —— 在 Slack、Notion、Gmail、Linear、Jira、HubSpot 之上构建 RAG / agent 能力
- **多 agent 与自动化工作流作者** —— 需要确定性的、可调度的唤醒, 而不是一路贯穿的 LLM 循环
- **强审计场景** —— 律所、银行、券商、医院这类需要追溯每个决策来源、按合规要求保留逐条事实与上下文修正记录的团队
- **开源 AI 建设者** —— 希望有一个 Apache-2.0、可自托管、能替代厂商记忆与上下文的技术方案
