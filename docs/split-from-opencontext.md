# Split from opencontext

This document records the history of carving the runtime sub-project
out of [opencontext](https://github.com/melandlabs/opencontext) into its
...
in the opencontext repo. The packages were extracted one leaf at a time,
with no behavioral change at each step, so that opencontext could keep
shipping while the carve-out was in progress.

### Phase 0 — Define the boundary types

Commit [`a25a5e21`](https://github.com/melandlabs/opencontext/commit/a25a5e21)
introduced `@opencontext/contracts` (now `@opencontext/contracts`). It
...
The motivation was that opencontext's backend code could not import a
...
Commit [`1d4d4ea4`](https://github.com/melandlabs/opencontext/commit/1d4d4ea4)
broke the static `@opencontext/indexeddb` coupling inside
`@opencontext/memory-store`. The store had previously imported types from
the IndexedDB package directly, which forced every consumer to drag in
browser globals. After this commit, `@opencontext/memory-store` declared
a structural `RawMessage` type and `@opencontext/indexeddb` became an
optional peer.

### Phase 2 — Migrate `UserType` consumers

Commit [`dc194290`](https://github.com/melandlabs/opencontext/commit/dc194290)
moved every consumer of the legacy `UserType` constant to import it
from `@opencontext/contracts` instead of from the app's auth module.

### Phase 3 — Migrate `IntegrationId` consumers

Each integration-specific module in the opencontext UI had been importing
`IntegrationId` from a local file. They were all updated to import
from `@opencontext/contracts/integration-id`.

### Phase 4 — Extract the database primitives

Commit [`044a3cc2`](https://github.com/melandlabs/opencontext/commit/044a3cc2)
introduced `@opencontext/db` with the dep-free leaf utilities
...
Commits
[`9408d1a6`](https://github.com/melandlabs/opencontext/commit/9408d1a6),
[`3da638e1`](https://github.com/melandlabs/opencontext/commit/3da638e1),
and [`90b2bc0e`](https://github.com/melandlabs/opencontext/commit/90b2bc0e)
extracted the dep-free leaves of the Loop engine (`paths`, `cli-path`,
`preferences`) and the Cron engine (`types`, `scheduler`,
`stream-response`) into `@opencontext/loop` and `@opencontext/cron`. The
...
Commit [`d84ac5a8`](https://github.com/melandlabs/opencontext/commit/d84ac5a8)
...
Commit [`1929e466`](https://github.com/melandlabs/opencontext/commit/1929e466)
...
callback script) into `@opencontext/integrations-runtime`.

### Phase 8 — Split env/config + remove Tauri from shared

Commits [`9ee0b748`](https://github.com/melandlabs/opencontext/commit/9ee0b748)
and [`cecc7942`](https://github.com/melandlabs/opencontext/commit/cecc7942)
introduced `@opencontext/env-config` (env/deployment-mode/Tauri-path
constants) and `@opencontext/ui-runtime` (Tauri platform detection +
browser/Tauri filesystem adapters). `@opencontext/shared` stopped
...
Commit [`b32b3039`](https://github.com/melandlabs/opencontext/commit/b32b3039)
tried to restructure the new packages into `runtime/` and `ui/`
top-level directories inside opencontext. This was rolled back in
[`d7b68ade`](https://github.com/melandlabs/opencontext/commit/d7b68ade)
...
moved from `opencontext/packages/*` to `opencontext/packages/*`, renamed
from `@opencontext/*` to `@opencontext/*`, and re-published under the
...
- `skills/` — Claude Code skills for the opencontext companion.
...
If you depend on `@opencontext/memory-store` (or any other moved
package), the migration is a rename:

```bash
# Before
pnpm remove @opencontext/memory-store
...
- The path constants that used to live under `~/.opencontext/loop/` now
...
pnpm -r test && pnpm -r lint` against the opencontext monorepo's
  pinned versions.

## Credits

The shape of this split was discussed and re-shaped over several
months by the opencontext maintainers. The Phase 9 rollback was the
moment we realised that the right place for the runtime was its own
repository, not a sub-directory of opencontext.
