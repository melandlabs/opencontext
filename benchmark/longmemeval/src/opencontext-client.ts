...
export async function checkOpencontextHealth(
  ...
  try {
    ...
    throw new Error(
      `OpenContext memory daemon not reachable at ${baseUrl}. Start it with: ` +
...
export async function ingestMessages(
  ...
  for (let i = 0; i < messages.length; i += INGEST_BATCH_SIZE) {
    ...
    if (!res.ok) {
      throw new Error(
        `ingest /v1/raw-messages failed: ${res.status} ${await res.text()}`,
      );
    }
...
export async function generateAnswer(
  ...
  if (!openrouterKey) {
    throw new Error(
      "No answerer LLM configured: set ANTHROPIC_AUTH_TOKEN (MiniMax) or OPENROUTER_API_KEY.",
    );
  }
