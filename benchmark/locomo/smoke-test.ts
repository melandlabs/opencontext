...
for (const sample of samples) {
	...
	console.log(`  ${sample.sample_id}: ${qaCount} QA, ${nonEmpty} non-empty, cats=${JSON.stringify(cats)}`);
}
...
for (const sample of samples) {
	...
	const hasNonEmpty = sample.qa_pairs.some((qa) => qa.answer && qa.answer.trim().length > 0);
